...
class deepwalk:
    ...
    def run(self):
        ...
        for path in self.__edge_list_path:
            ...
            command = "deepwalk --format edgelist --input \"%s\" --representation-size %d --number-walks 10 --output \"%s\"" \
                      % (path, self.__representation_size, output_path)
            print(command)
            os.system(command)
    ...
    def get_ebd(self, tmp_file_dict):
        embeddings = self._get_embedding(tmp_file_dict)
        embedding_path_dict = self._output_embedding_as_csv(embeddings)
        return embedding_path_dict
        ...
        for file_name in tmp_file_dict:
            ...
            for i in range(self.__nodes_num):
                if i+1 in indexList:
                    ...
                    result.append([random.uniform(-1,1) for i in range(self.__representation_size)])
